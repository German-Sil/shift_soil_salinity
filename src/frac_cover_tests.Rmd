...
frac_8 <- read_stars(here("imagery", "subset_images", "rfl_2022_04_20.tif_SMA_20240201T11h21M37S")) %>% 
  filter(band %in% c("NPV_fraction", "GREEN VEG_fraction", "BARE SOIL_fraction")) %>% 
  split(3) %>% 
  setNames(c("NPV_fraction", "GREEN_VEG_fraction", "BARE_SOIL_fraction"))
...
lm(electro_cond_mS_per_cm ~ elevation + mari + vssi + crsi + ndvi + GREEN_VEG_fraction + BARE_SOIL_fraction + NPV_fraction,
                      data = soils_extract) %>% 
  car::vif()
