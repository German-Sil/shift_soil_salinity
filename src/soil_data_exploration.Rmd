...
all_corr
```
## NIR Version Needs Debugging/Parameterization

```{r}

fit_nir <- train(electro_cond_mS_per_cm ~ elevation + ndvi + mari + vssi + crsi + `76` + `77` + `78` + `79` + `80` + `81` + `82` + `83` + `84` + `85` + `86` + `87` + `88` + `89` + `90` + `91` + `92` + `93` + `94` + `95` + `96` + `97` + `98` + `99` + `100` + `101` + `102` + `103` + `104` + `105` + `106` + `107` + `108` + `109` + `110` + `111` + `112` + `113` + `114` + `115` + `116` + `117` + `118` + `119` + `120` + `121` + `122` + `123` + `124` + `125` + `126`, 
                 data = soils_extract, 
                 method = "rf", 
                 trControl = trainControl(method = "cv", number = 10), 
                 importance = TRUE)
...
```{r}
# using caret
fit_nir_imp <- as.data.frame(fit_nir$finalModel$importance)
fit_nir_imp_scaled <- predict(preProcess(fit_nir_imp, method = c("range")), fit_nir_imp) %>% 
  rownames_to_column(var = "variable")

ggplot(fit_nir_imp_scaled) +
  geom_point(aes(x = IncNodePurity, y = variable), color = "#E69512")+
  geom_segment(aes(y= variable, x = 0, yend = variable, xend = IncNodePurity), color = "#E69512", size = 1.2)+
  geom_point(aes(x = fit_nir_imp_scaled$`%IncMSE`, y = variable), color ="#3A5D3D")+ 
  geom_segment(aes(y= variable, x = 0, yend = variable, xend = fit_nir_imp_scaled$`%IncMSE`), color = "#3A5D3D", linetype = "dashed")+
   labs(y = "Variables",#labels
       x = "Importance (Scaled)")+
  ggtitle("Random Forest Variable Importance")+#plot title
  theme(plot.title = element_text(color = "#5b4f41", size = 16, hjust = 0.5),#plot themes
...
cor(fit_nir$finalModel$y, fit_nir$finalModel$predicted)
```
...
ggplot()+#ggplot object
  geom_abline(slope = 1, intercept = 0, linetype = "dotted")+
  geom_point(aes(y = fit_nir$finalModel$y, x = fit_nir$finalModel$predicted), color = "#E69512")+#data to make points from
  geom_smooth(method = "lm", aes(y= fit_nir$finalModel$y, x = fit_nir$finalModel$predicted), color =  "#3A5D3D")+#smooth line data
  scale_x_continuous(expand = c(0, 0), limits = c(0, 14)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 14))+
  labs(y = "Actual Electrical Conductivity (mS/cm)",#labels
       x = "Predicted Electrical conductivity (mS/cm)")+
...
nir_corr <- cor(y= fit_nir$finalModel$y, x = fit_nir$finalModel$predicted)
nir_corr
```
...
raster_all_nir <- c(raster_all, nir_all, along = "date") %>% 
  merge()
...
ggplot()+
  ...
  scale_fill_gradientn(colors = rev(calecopal::cal_palette(name = "desert", n = 2, type = "continuous")))+
...
# Predicting values (NIR)

```{r}
nir_pred <- predict(raster_all_nir, fit_nir)
...
```{r}
nir_pred_feb <- nir_pred %>% 
  filter(date == "2022-02-24")

ggplot()+
  geom_stars(data = nir_pred_feb, aes(x, y, fill = na.omit(prediction))) +
