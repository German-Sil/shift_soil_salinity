...
```{r}
soils <- cbind(soils, dem_extract$dem.tif) %>% #bind extracted values to the soil data
  rename("elevation" = "dem_extract.dem.tif") %>% 
  st_transform(crs = 32611)
```
...
```{r}
fit_ele <- randomForest(electro_cond_mS_per_cm ~ elevation, data = soils, importance = TRUE)#random forest fitting of the data with stated equation

ggplot()+#ggplot object
  geom_point(aes(x= soils$electro_cond_mS_per_cm, y = fit_ele$predicted), color = "#E69512")+#data to make points from
  geom_smooth(method = "lm", aes(x= soils$electro_cond_mS_per_cm, y = fit_ele$predicted), color =  "#3A5D3D")+#smooth line data
...
elevation_cor <- cor(x= soils$electro_cond_mS_per_cm, y = fit_ele$predicted)#calculate correlation coefficient
...
for (i in unique(lubridate::week(soils$paired_flight))){
  ...
  soils_week <- bind_cols(soil_week, ndvi_extract$ndvi, mari_extract$mari, vssi_extract$vssi, crsi_extract$crsi) %>% 
    rename("ndvi" = "...11",
           "mari" = "...12",
           "vssi" = "...13",
           "crsi" = "...14")
    
  soils_si <- rbind(soils_si, soils_week)
}
...
fit_all <- train(electro_cond_mS_per_cm ~ elevation + ndvi + mari + vssi + crsi, data = soils_si, method = "rf", trControl = trainControl(method = "cv", number = 10)) #random forest fitting of the data with

trainx <- soils_si %>% 
  as.data.frame() %>% 
  select(elevation, ndvi:crsi)

fit_all_1 <- randomForest(electro_cond_mS_per_cm ~ elevation + ndvi + mari + vssi + crsi, data = soils_si, importance = T)

importance(fit_all_1)

fit_all_1_cv <- rfcv(trainx = trainx, trainy = soils_si$electro_cond_mS_per_cm, cv.fold = 10, scale = "log")
...
rf_imp <- importance(fit_all_1)
rf_imp_scaled <- predict(preProcess(as.data.frame(rf_imp), method = c("range")), as.data.frame(rf_imp))
...
```{r}
ggplot()+#ggplot object
  geom_point(aes(x= fit_all$finalModel$y, y = fit_all$finalModel$predicted), color = "#E69512")+#data to make points from
  geom_smooth(method = "lm", aes(x= fit_all$finalModel$y, y = fit_all$finalModel$predicted), color =  "#3A5D3D")+#smooth line data
  labs(x = "Actual Electrical Conductivity (mS/cm)",#labels
       y = "Predicted Electrical conductivity (mS/cm)")+
...
all_corr <- cor(x= fit_all$finalModel$y, y = fit_all$finalModel$predicted)
all_corr
```
...
```{r}
ggplot()+#ggplot object
  geom_point(aes(x= fit_all_1$y, y = fit_all_1$predicted), color = "#E69512")+#data to make points from
  geom_smooth(method = "lm", aes(x= fit_all_1$y, y = fit_all_1$predicted), color =  "#3A5D3D")+#smooth line data
  labs(x = "Actual Electrical Conductivity (mS/cm)",#labels
       y = "Predicted Electrical conductivity (mS/cm)")+
  ggtitle("Elevation and Indices (randomForest method)")+#plot title
  theme(plot.title = element_text(color = "#5b4f41", size = 16, hjust = 0.5),#plot themes
        plot.background = element_rect("white"),
        panel.background = element_rect("#faf7f2"),
        panel.grid = element_line(linetype= "longdash", color = "#f0ece1"),
...
#ggsave(filename = "all_plot_rf.png", plot = last_plot(), width = 12, height = 8, device = "png", dpi = 500) #write an image of the plot

all_corr <- cor(x= fit_all_1$y, y = fit_all_1$predicted)
all_corr
```
...
fit_si <- train(electro_cond_mS_per_cm ~ ndvi + mari + vssi + crsi, data = soils_si, method = "rf", trControl = trainControl(method = "cv", number = 10)) #random forest fitting of the data with
...
cor(fit_si$finalModel$y, fit_si$finalModel$predicted)
...
```{r}
ggplot()+#ggplot object
  geom_point(aes(x= fit_si$finalModel$y, y = fit_si$finalModel$predicted), color = "#E69512")+#data to make points from
  geom_smooth(method = "lm", aes(x= fit_si$finalModel$y, y = fit_si$finalModel$predicted), color =  "#3A5D3D")+#smooth line data
  labs(x = "Actual Electrical Conductivity (mS/cm)",#labels
       y = "Predicted Electrical conductivity (mS/cm)")+
...
si_corr <- cor(x= fit_si$finalModel$y, y = fit_si$finalModel$predicted)
si_corr
```
