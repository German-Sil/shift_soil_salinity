...
Conductivity 
(mS/cm)")+
  ggtitle("May 29, 2022")+#plot title
...
for (i in unique(lubridate::week(soils_subset$paired_flight))){
  ...
  upland_week <- up_frac_all %>% 
    filter(lubridate::week(date) == i)
  
  pick_week <- pick_frac_all %>% 
    filter(lubridate::week(date) == i)
  
  bsoil_week <- soil_frac_all %>% 
    filter(lubridate::week(date) == i)
  ...
  crsi_extract <- crsi_week %>% 
   ...
   upland_extract <- upland_week %>% 
    st_extract(soil_week)
  
   pick_extract <- pick_week %>% 
    st_extract(soil_week)
   
   bsoil_extract <- bsoil_week %>% 
    st_extract(soil_week)
