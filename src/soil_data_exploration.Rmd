...
fit_all <- train(electro_cond_mS_per_cm ~ elevation + ndvi + mari + vssi + crsi, data = soils_si, method = "rf", trControl = trainControl(method = "cv", number = 10), importance = T) #random forest fitting of the data with
...
#ggplot(rf_imp_scaled) +
#  geom_segment(aes(y= variables, x = 0, yend = variables, xend = IncNodePurity), color = "#E69512", size = 1.2)+
#  geom_segment(aes(y= variables, x = 0, yend = variables, xend = rf_imp_scaled$`%IncMSE`), color = "#3A5D3D", linetype = "dashed")+
#  geom_point(aes(x = IncNodePurity, y = variables), color = "#E69512", size = 1.5)+ 
#  geom_point(aes(x = rf_imp_scaled$`%IncMSE`, y = variables), color ="#3A5D3D")+ 
#   labs(y = "Variables",#labels
#       x = "Importance (Scaled)")+
#  ggtitle("Random Forest Variable Importance")+#plot title
#  theme(plot.title = element_text(color = "#5b4f41", size = 16, hjust = 0.5),#plot themes
...
#  scale_y_continuous(expand = c(0, 0), limits = c(0, 13))+
#  geom_abline(slope = 1, intercept = 0, linetype = "dotted")+
#  geom_point(aes(y= fit_all_1$y, x = fit_all_1$predicted), color = "#E69512")+#data to make points from
#  geom_smooth(method = "lm", aes(y = fit_all_1$y, x = fit_all_1$predicted), color =  "#3A5D3D")+#smooth line data
#  labs(y = "Actual Electrical Conductivity (mS/cm)",#labels
#       x = "Predicted Electrical conductivity (mS/cm)")+
...
#all_corr <- cor(y= fit_all_1$y, x = fit_all_1$predicted)
#all_corr
```
...
fit_si <- train(electro_cond_mS_per_cm ~ ndvi + mari + vssi + crsi, data = soils_si, method = "rf", trControl = trainControl(method = "cv", number = 10), importance = T) #random forest fitting of the data with
